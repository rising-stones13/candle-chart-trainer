...
import type { CandleData, Trade, MAConfig, MacdData, LineData, PositionEntry, VolumeConfig } from '@shared/types';
...
export function StockChart({
  ...
  const { chartContainerRef, chartRef, isChartInitialized } = useLwcChart(
    getChartOptions(upColor, downColor, chartTitle) as TimeChartOptions
  );
  ...
  useEffect(() => {
    ...
    seriesRef.current.macdLine = chart.addLineSeries({ 
      priceScaleId: 'macd', 
      color: '#2196F3', 
      lineWidth: 2, 
      lastValueVisible: false, 
    ...
    seriesRef.current.macdSignal = chart.addLineSeries({ 
      priceScaleId: 'macd', 
      color: '#FF5252', 
      lineWidth: 2, 
      lastValueVisible: false, 
  ...
  useEffect(() => {
    ...
    if (seriesRef.current.volume) {
      const volumeData = currentData.map(d => ({ 
        time: d.time, 
        value: d.volume, 
        color: d.close >= d.open ? 'rgba(8, 153, 129, 0.5)' : 'rgba(239, 83, 80, 0.5)' 
      }));
      ...
      chart.priceScale('volume').applyOptions({ visible: volumeConfig.visible });
    }
    ...
    if (seriesRef.current.rsi) {
      ...
      chart.priceScale('rsi').applyOptions({ visible: rsiData.length > 0 });
    }
    ...
    if (seriesRef.current.macdLine && seriesRef.current.macdSignal && seriesRef.current.macdHistogram) {
      ...
      const histogramData = macdSlice.map(d => ({ 
        time: d.time, 
        value: d.histogram, 
        color: d.histogram && d.histogram > 0 ? upColor : downColor 
      ...
      chart.priceScale('macd').applyOptions({ visible: isVisible });
    }
  ...
  }, [isChartInitialized, chartData, replayIndex, maConfigs, rsiData, macdData, upColor, downColor, volumeConfig, isPremium, chartRef]);
